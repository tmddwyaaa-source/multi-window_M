---
name: multi-window-m-035
description: >-
  多窗口分工：常驻仅 M1～M10；短期任务用临时窗 C1、C2（一轮最多 4 个）。
  窗号规则见正文「窗口身份」；不要发明第三套编号。
  旧项目 migrate-project 必须先备份并出迁移报告，写入 skill_version / taskctl_version / 迁移时间；
  --check 过基本门禁、Full Gate、Hook、负向后才继续开发。
  开局自报三套挡位；未确认禁止报加分。M1 唯一收口；查收以本窗重跑为准。
  规则只描述当前行为；门禁细节见 references/task-gate.md，宿主接线见 references/hooks.md。
  斥候→主力→搜剿；同一卡点最多 3 次（第 2 次换负责人，第 3 次硬停）。
  在用户提到多窗口、M1、C1、查收、模块注册表、斥候/主力/搜剿时使用。
version: "0.35"
disable-model-invocation: true
---

# Multi-Window_M-0.35（常驻 M1～M10，临时 Cn）

本文件夹名带版本号，是升级系列的隔离副本；`version` 字段也是 0.35。调用：`/multi-window-m-035`——宿主把 `/调用名` 与 `name` 字段逐字符比对（且只接受小写字母、数字、连字符），所以这里不带下划线也不带点。历史版本说明见 [CHANGELOG.md](CHANGELOG.md)，不得当作当前规则。

复制即用话术见 [templates.md](templates.md)。G0～G3、manifest schema、`transition` 表见 [references/task-gate.md](references/task-gate.md)。宿主接线（Cursor / Codex / Zcode / dsh）只允许写在 [references/hooks.md](references/hooks.md)；dsh 的装/验/撤步骤见 [references/dsh-evidence.md](references/dsh-evidence.md)。

## 本版唯一问题与成功标准

**问题：** 窗号禁忌散落多处、只写「禁止」、并曾把旧文件名前缀当成一种编号来教。

**成功标准：**

- SKILL 里「窗口身份（窗号）」是唯一权威；词法以 `taskctl.py` 的 `WINDOW_RE` / `valid_window()` 为准，本节是镜像。
- 合法窗号只有 `M1`～`M10` 与 `C`+数字。不新开第三套编号。产出用普通文件名。
- 铁规则 1～2、旧「编号」表、反模式、templates 头、frontmatter 只指向该节，不再复述禁令清单。
- 行为不变：`valid_window()` 与 `round-init`（非法窗号 / 重复窗号 / 一轮临时 C 超过 4）照旧。不新增失败 token。

0.31 的迁移治理、0.30 的挡位与 hook 证据、0.29 的 `source_refs` 仍是当前规则。本版不改状态机转移表或 Hook 三不。

## 核心认知（三条铁规则）

1. **窗号**：只使用「窗口身份」里的合法窗号。对窗口说「我是 M4」或「我是 C1」。详见下一节。
2. **文件名不是窗号**：查收、打回、RECEIPT-LOG 只用窗号。产出文件用普通路径，不要发明第三套编号。详见下一节。
3. **查收看命令过没过**：验收命令**只检查工人已经写在磁盘上的文件**，不负责生成那些文件。M1 按该窗验收命令**本窗重跑**。一行 `RESULT PASS` 只要是命令打出来的，就是全文。窗口**不会**自动把结果交给终端，也**不会**自动叫醒 M1。

## 窗口身份（窗号）

窗口身份是一个**封闭**的两段命名空间：

    M1 .. M10  常驻（一段）     C1, C2, …  临时（一段）

**词法（代码是源，本节是镜像）：** 合法窗号 = `M[1-9] | M10 | C[1-9][0-9]*`。判定以 `taskctl.py` 的 `WINDOW_RE` / `valid_window()` 为准。禁止把本节改成与代码不一致。下文实例都是「不是合法窗号」或「违反本轮协议」，不是另外一套规则。

**为什么封闭：** M1 唯一收口、Registry 路由、查收、RECEIPT-LOG、窗口状态都以「窗号 ∈ 该集合」为键。写成集合外标识 ⇒ 键失准。

| 写错 | 属于 | 为什么非法 |
|------|------|-----------|
| M11, M12… | 常驻段溢出 | 常驻只有 M1～M10；`WINDOW_RE` 不接受 M11 |
| F2 | 新增类别 | 只有 M/C 两套，不新开第三套窗 |
| 窗口 8 | 非规范描述 | 必须用 `M{n}` / `C{n}` |

**协议（非词法）：**

- **派工：** `round-init` 拒绝非法窗号、重复窗号、以及一轮超过 **4** 个临时 C（现有行为，本版不改）。
- **查收：** 只用本轮派工名单；临时 C 查收通过后关闭，号码下一轮可复用。代码不代标 done。

**产出文件：** 用普通路径（如 `data/words-batch/batch-02.json`）。文件名不是窗号。不要为文件另编一套窗号。

**例外：** 历史项目里已 `done` 的 M11～M15 **保持原样、不改名**（改名断引用）。只约束新开窗。旧磁盘上的异常文件名当文件看，不当窗号，新模板不再使用。

同时生效：

- **空间**：M1 / 常驻 Mn / 临时 Cn 各守路径。**阶段**：斥候 / 主力 / 搜剿。
- **只有 M1** 能把模块标 `done`。子窗口搜剿通过 ≠ 全局完成。
- 用户说「所有窗口已完成」= **本轮派工**名单（其中的 M 与 C），不是历史上所有编号。
- 该窗未送达查收信号前，不得标 done（信号可以是口播、粘贴、已点名的关联会话、子代理回传）。
- **「亲自微修订」必须用户点名**。没点名就派对应 M 或 C。用户对 M1 说「开工 / 修一下 / 顺手」= 写 Registry + 开工话术，不等于本窗当主力。
- **网页预览仅 M1**。子窗口禁止打开网页预览。查收通过后能开则 M1 打开；打不开则跳过，不因此 fail。

### 环境：三套挡位（开局自报）

本 skill 不绑死某一款 Agent 软件。三套挡位正交，只影响协作方式，**不影响验收纪律**（本窗重跑、`transition` 收口、M1 唯一收口在任何挡位不变）。不确定一律落低挡。

| 挡位 | 取值 | 判定 |
|------|------|------|
| **能力** | 默认 / 加分（bonus） | 本窗已确认的额外能力；未确认 = 默认。前 3 轮点名加分，不确定只问不报 |
| **协作** | P 人路由多窗 / A M1 子代理半自动 | P 是所有宿主起点且永久保留；A 仅限 `capability=bonus` 且 `capability_confirmed=true` |
| **风险** | 短路径 / 独立验收 | 唯一策略表：low 且 attempt<2 走短路径，其余独立验收 |

能力确认测试命令按宿主写在 `references/hooks.md`。新窗口跑一次、本窗实际收到结果才算确认。说明书上有子代理、本窗还没用上，**不是**加分，也**不能**把协作挡写成 A。

`round.json` 用 `gears` 记录能力挡与协作挡；风险挡仍由各任务 `risk` / `attempt` / `verification_required` 决定。未写 `gears` = 默认 + P。未确认却写 bonus 或 A → `GEAR_VIOLATION`。

加分只看**这一窗已经确认**的额外能力。窗间对话是否可见，各宿主不一样；窗间不可见也不等于不能加分——仍可能有别的额外能力（例如能把子代理结果收回本会话）。

| | **默认（P）** | **加分（可升 A）** |
|--|----------|------|
| 何时 | 所有环境的起点；未确认额外能力时全程按此 | 本窗已确认存在超出默认的额外能力 |
| 看见什么 | 不假设能看见其他会话的聊天原文 | 已确认的关联会话 / 子代理回传可作为**线索** |
| 协作 | 同一项目路径 + `docs/` + 用户把查收信号送到 M1 | 可少传话、少开窗；子代理必须写入 `subagents` 绑定 |
| 查收 | 本窗重跑该窗验收命令 | 同样必须本窗重跑。不能只凭「那边说完成了」标 done |
| 出句 | **不要说加分**，直接派工 | 前 3 轮必须点名「当前是加分状态」并自报三套挡位 |

**出句（硬性，只约束 M1）：** 开局先报本窗挡位。只有判定为加分才准贴加分句。默认禁止说加分。不确定则前 3 轮内只问、禁止先报加分；未确认前按默认。前 3 轮没点名 → 本窗全程按默认。句式见 templates.md（加分句不是默认开场）。

**默认流程（所有宿主的起点）：** M1 派工写 Registry 与 manifest → 跑 `brief` 把生成物交给工人 → **用户**开 M/C 窗并说「我是 {窗号}」→ 工人按简报改文件并**自己在本窗终端跑验收命令** → **用户**对 M1 说「{窗号} 已完成，请查收」→ M1 **再跑同一条命令**。失败则 M1 打回，**用户**再到该工人窗说「按打回项继续」。没有「跑完自动交 M1」「失败自动重开」这一跳。新 M1 先跑 `handoff`，不要凭聊天记忆接班。

**派工纪律（省 M1 字数的关键）：** 派工内容 = **原样贴 `brief` 生成物**，M1 不另写说明。`brief` 就是"各窗口应接收的指示"的生成器；M1 在这上面省下的字，属于被重发最多次的那一类字节。

### 宿主适配：DSH（默认能力 + 协作挡 P）

- `gears` 固定 `capability=default`、`collaboration=P`、`capability_confirmed=false`；**不要打开 `hook_supervision`**（本技能接线后仍按默认+P；详见 [references/hooks.md](references/hooks.md)）。
- **子代理不得担任正式工人 / 替换负责人 / 验收者**：DSH 子代理读不到本 skill（`disable-model-invocation` 的必然结果），只可做受限斥候、检索、低风险辅助。派它时至少给一份极短**任务胶囊**（见 [templates.md](templates.md)）。
- 因此 DSH 上第 2 级阶梯通常走不到：**没有合格的新执行者时写 `NO_ELIGIBLE_REPLACEMENT_WORKER`**，请 M1 / 用户开新窗，不得伪造换人。
- 正式实施、独立验证、M1 收口，一律由**已加载本 skill、身份明确的 M/C 窗口**承担。

## 窗口角色（空间）

| 角色 | 职责 |
|------|------|
| **M1** | 大纲、Registry、查收、集成、RECEIPT-LOG；能开则打开网页预览。亲自改子模块必须用户点名且同意 |
| **M2～M10** | 常驻模块执行人：各守一块长期模块；环内斥候→主力→搜剿；禁止打开网页预览。已 `done` 的不必常开 |
| **C1、C2…** | 临时短任务（如一批词库）。只改本轮规定路径；查收通过后关掉。号码下一轮可复用。一轮最多 **4** 个（见「窗口身份」） |
| **用户** | 开窗、传开工话术、把查收信号送到 M1；实测 |

历史已 done 的溢出常驻编号：见「窗口身份」例外。

## 阶段角色（斥候 / 主力 / 搜剿）

| 名称 | 允许 | 禁止 |
|------|------|------|
| **斥候** | 只读调查、列路径与可疑点、建议最小范围 | 改代码、声称已修好、改 Registry 为 done |
| **主力** | 最小改动实现；交计划、diff、本回合终端原文 | 越界；自己标 done；打开网页预览；只交 PASS 摘要；指望「脚本替我生成交付文件」 |
| **搜剿** | 只验收挑刺；施工窗没跑终端则 fail；M1 以本窗复跑为准 | 凭感觉放行；顺手改子模块；因没有子窗 stdout 打回 |

环内：斥候→主力→搜剿（**斥候按需触发，见「性能纪律」第 4 条**）；搜剿 fail 则主力↔搜剿；**同一卡点最多 3 次**，第 2 次换真实负责人，第 3 次硬停写升级报告（见「长线治理」）。完工话术：「{窗号} 已完成，请主导窗口查收。」

## 对窗说话（详见「窗口身份」）

| 字段 | 规则 |
|------|------|
| **章节标题** | `## M4 — 背单词` 或 `## C1 — 词库批次` |
| **对工人说** | `我是 M4` / `我是 C1` |
| **对 M1 说** | `M4 已完成，请查收` / `C1 已完成，请查收` |
| **产出文件** | 普通路径，例如 `data/words-batch/batch-02.json`。文件名不是窗号 |

## 本轮派工

```markdown
## 本轮派工
- 本轮窗口：M4、C1、C2
- 本阶段不使用：M6、M7
- 临时窗上限：本轮 2 个（最多 4）
```

无「本阶段不使用」可写「无」。未列入本轮的编号不得写成缺失或 fail。

## 任务控制（当前规则）

没有 `.task/` 的项目仍按上面的多窗口流程工作。启用 `.task/` 后：

- **唯一策略实现**是 `scripts/taskctl.py` 的 `verification_policy()`。Gate、`audit-round`、`transition`、M1 收口禁止各自解释。
- **任务状态**写在 `.task/TASK-xxx/manifest.json`。**窗口状态**写在 `.task/round.json` 的 `window_status`。`taskctl status` 可在窗口行旁标注对应任务终态，**不改写** `window_status`。应写成 `TASK-001: verified`、`M2: worker_done`。
- 低风险且 `attempt < 2` 且未设 `verification_required`：短路径 `worker_done → integrated`（`--actor M1`）→ `done`。其余：独立验收 `worker_done → verifying → verified → integrated → done`。同一时刻只允许走其中一条。
- 工人只能交 `worker_done`。需要独立验收时，仅 verifier 可交 `verifying` / `verified`。只有 M1 能 `integrated` / `done`。直接编辑 manifest 的状态不算有效收口。
- 规则冲突（无法唯一决定收口路径，或 manifest 自相矛盾）→ 输出 `POLICY_CONFLICT` 并停止。
- Full Gate 会重跑可识别的验收命令、核 evidence 路径、有 Git 时核工作区。`manifest.json` / `rerun.json` / `verify-report.json` 不必列入工人 `changed_files`。
- `--root` 必须放在子命令前面。常用命令与 schema 见 `references/task-gate.md`。
- 一键自检（不依赖写死的本机路径）：`py -3 scripts/taskctl.py selftest`。
- **派工简报：** `py -3 scripts/taskctl.py --root <项目根> brief TASK-xxx --role worker|scout|verifier`。M1 把终端输出原文贴给该窗或子代理，不要手写转述代替生成物。
- **接班简报：** `py -3 scripts/taskctl.py --root <项目根> handoff`。新 M1 先跑这一条再动手。不存在的 task 或非法 `--role` 会被拒绝。
- **状态视图：** `py -3 scripts/taskctl.py --root <项目根> status --markdown --write`。写入 `docs/TASK-STATUS.md`。禁止手改该文件；不要在 Registry / RECEIPT-LOG 里另写一套 pending/done 当权威。`status` 是**只读视图**：它**不执行任何 shell 命令**，只显示上次真跑 Gate 的记录（`GATE_PASS` / `GATE_FAIL` / `GATE_NOT_RUN` / `GATE_STALE`）。它**不是**收口依据。要真跑就显式跑 `gate --full` 或 `audit-round`；确实需要在一个命令里逐任务重算时才加 `--deep`（会执行回归命令，可能几分钟）。
- **关轮：** `py -3 scripts/taskctl.py --root <项目根> round-close`。只有**整轮任务全部 `done` 且审计通过**才归档 `round.json` 到 `.task/rounds/`；未完成任务一律拒绝。绝不自动触发，**禁止手工删除 `round.json` 开新轮**。归档后正常跑 `round-init` 开下一轮。
- **需求覆盖：** 用户每条新需求写入 `source_refs`（或 round 的 `source_requirements`）并映射到带 `verify`/`verify_cmd` 的 R 项。缺映射 → `REQUIREMENT_COVERAGE_FAIL`，停止收口。禁止只改聊天话术。
- **挡位：** `round.json` 的 `gears` 记录能力挡与协作挡。未写 = 默认 + P。未确认的 bonus / A → `GEAR_VIOLATION`。
- **Hook 证据：** `hook_supervision=true` 时，收口必须有宿主 stop 的 start/end 对：Cursor / Codex / Zcode 看 `hook-runs.jsonl`，dsh 看 `dsh-runs.jsonl`（见 `references/hooks.md` 与 `references/dsh-evidence.md`）。缺日志或只有 manual → `HOOK_EVIDENCE_MISSING`。只有**产品本身没有 hook 能力**的宿主才不要打开 `hook_supervision`，以免卡死最低挡；dsh 已接线，不在其列。
- **子代理绑定：** 派出的子代理写入 `subagents`（`task_id` / `window` / `allowed_paths` / `run_id`）。同文件并发、重复、工人兼 verifier、越权 → `PARALLEL_FAIL`。

旧项目升级：用**已安装的本技能脚本**执行 `migrate-project --destination scripts/taskctl.py --force`（先备份再覆盖），再跑 `migrate-project --check`。未出现 `MIGRATE_READY` 前不要开发新功能。项目记录在 `.task/skill-lock.json`，报告在 `docs/MIGRATE-REPORT.md`。

### `.task/` schema 版本（跨宿主契约）

`manifest.json` 与 `round.json` 有顶层 `schema_version`（当前 `1`）。**`skill_version`（技能行为版本）与 `schema_version`（共同文件契约版本）是两件事**，技能升版不一定动 schema。

- 读到**更高的** `schema_version` → 输出 `UNSUPPORTED_SCHEMA`，**只允许查看**：禁止改状态、禁止收口、禁止任何写操作。这是 fail closed，防止旧宿主用自己旧的理解静默改写新宿主写出的数据。
- 读到**缺失或更低的** `schema_version` → 视为旧文件，走迁移兼容路径继续工作（迁移报告里标 `LEGACY_UNSCOPED`）。
- 旧 manifest 缺 `block_attempts` 时，卡点计数回落到任务级 `attempt`：这是**保守兼容**，旧项目不会因为缺字段凭空多出重试次数。

**反方向：`SCHEMA_REGRESSION_RISK`（被更旧的宿主操作过）。**

`UNSUPPORTED_SCHEMA` 防的是"未来版写的数据被现在这版乱改"，它**防不住反方向**：旧宿主完全不认识 `schema_version`，所以它既不维护这个标记、也不报错，会若无其事地在这个项目里开新轮、改状态。

判据（机械可查）：`.task/skill-lock.json` 已声明契约版本 N，而**活动轮或某个任务目录**的 `schema_version` < N → 说明这些文件不是当前版本写的 → 输出 `SCHEMA_REGRESSION_RISK` 并**拒绝一切写操作**。只读命令（`status` / `gate` / `audit-round` / `brief` / `packet` / `handoff`）仍然可用。

- **真正的历史项目不受影响**：锁里也没有版本声明时（N=0），缺标记就是正常的旧文件，继续兼容。
- 契约只在**写了新文件之后**升级（`init` / `round-init` / `migrate-project`），**绝不在读取路径上自动补写**——否则第一次读取就把契约补上，检测随之失效。
- 检测覆盖**活动**文件；已归档到 `.task/rounds/` 的轮和已关闭的任务不在保护范围内。

**为什么这件事重要（回退的具体破坏机制，实测确认）：**

1. 旧宿主（0.34）源码里 `schema_version` **0 处命中**——它读不懂这个标记，所以不会拒绝。
2. 它的 `round-init` **不写**该字段 → 新建的当轮在现行宿主眼里"看起来是旧文件" → 继续无保护地干活。
3. 每个任务因此**永久**获得 `current_attempts()` 的**回落路径**（梯级计数从任务级历史起算）——规则被静默降级。
4. 旧宿主的 `transition` 会写回 manifest，而它不更新任何新字段：**它用的是旧规则，却留下了"看起来正常"的状态。**
5. 最坏情况：**同一份 `.task/` 下，新旧两个宿主对"该不该换人 / 该不该硬停"给出不同结论，而没有任何一方报错。**

结论：**0.35 项目不得回退用旧宿主操作。** 这不是保守建议，是上面五条的实际后果。

### 不可重放命令与过期报告（0.35 收口检查）

- **`UNREPLAYABLE_COMMAND`：** `tests[].command` / `verify_cmd` 若是**占位符**（`<临时脚本>`）、**带说明文字**（`node a.mjs (临时文件，已删除)`）、**中文全角标点**、**没有可识别运行器前缀**，或**引用了已声明却缺失的文件**，收口直接失败并指出条目。它是**验收证据错误**：不推进卡点计数、不算一次失败。纪律：临时探针写在 `note` 里，`tests[]` 只放**可执行且可重放**的命令。
  边界：**本机没装的运行器不算**（例如本机无 `pytest` 时 `pytest -q` 仍可重放），不做二进制存在性判定。
- **`STALE_REPORT`：** 报告自称的 `changed_files` 在报告之后又被改过 → 打提示。判据用**文件内容指纹**，不用 mtime（时间戳可能只有秒级粒度，同秒写入会假阴性）。
  **分层（重要）**：日常 `gate` 与 `audit-round` 只**提示**（不判 FAIL、不进卡点，避免打断改代码的循环）；**只有真正归档的那一步**（`round-close`）把它当**阻断项**——收口是"声称验收新鲜"的那个动作，不能拿过期证据过关。Hook 触发的审计不是收口，同样只提示。
  它只是**提示**，**不能**声称已保证验收新鲜：指纹基线是"上次 Gate 时的工作区内容"，门禁无法知道报告是谁写的、什么时候写的。

## Hook 边界

Hook 只是触发器，不是验收结论。有 `.task/` 时 `hook-audit` 写证据账本：Cursor / Codex / Zcode 写 `.task/hook-runs.jsonl`（`start`/`end` 成对，同一 `run_id`，最多 100 行）；dsh 写 `.task/dsh-runs.jsonl`（独立额度 400 行）。两个账本都要 gitignore。没有 `.task/` 时静默跳过。

`round.json` 的 `hook_supervision` 默认为 false。为 true 时，M1 跑 `audit-round` 收口必须看到完整一对，`source` 为 `cursor-stop` / `codex-stop` / `zcode-stop` / `dsh-stop` / `dsh-subagent-end`；`--source manual` 或只跑 `audit-round` **不算** hook 证据。未声明监督时，缺 jsonl 不卡死最低挡，但也不得把缺日志当成完成证据。

Hook **三不**：不改状态、不派工、不标 done。失败只记账。即使 Hook 输出成功，也不能替代 M1 本窗重新运行 Full Gate。

宿主差异（事件名、适配器、能力确认测试命令）只写在 `references/hooks.md`。本文件与 `taskctl.py` 不按宿主名分支行为。

## 派工用「就绪包」（DSH 方向，0.35）

用户只说一句「你是 Mn，请完成指示任务」，所以**窗口要读的东西必须自己就绪、且足够短**。M1 派工时用：

```text
py -3 scripts/taskctl.py --root <项目根> packet TASK-001 --role worker --write
```

产出 = 简报全部内容 **+ 读取预算 + 输出预算 + 归属确认**，并由 `--write` 落到 `.task/TASK-001/packet-<窗号>.md`。窗口读这一份即可，**不必再翻 Registry，也不必整篇读设计表**。

**读取预算**写在 `manifest.read_budget`，M1 派工前必须补：

```json
"read_budget": [
  {"path": "docs/设计表.md", "anchor": "## 2.4 升级池", "lines": "68-105", "why": "只改池"},
  {"path": "docs/GAME-SPEC.md", "lines": "full", "why": "短文件"}
]
```

- `lines` 只接受 `N-M` 或 `full`，其它值门禁直接拒（`lines must be 'N-M' or 'full'`）；`anchor` 可选，但给了就不能为空；
- **`anchor` 的作用**：行号会漂移，锚点让窗口确认自己读的是**正确章节**（代码文件可用函数名/类名）；
- 没声明时，就绪包会提示"未声明 read_budget，M1 派工前应补上"；
- **`read_budget` 是「减少无效上下文的派工约束」，不是「可完全证明的安全权限边界」**——它管的是"别乱读大件"，不是权限隔离；
- **`packet` 是可再生成的派工副本，禁止手改**：状态与约束一律以 `.task/` 的 manifest / round 为准。

**行号/锚点由 M1 查，不许向用户要**：M1 负责"确保读取范围已定位并写入"；可由斥候辅助定位；**绝不要求用户提供行号**（用户只描述改哪一块）。

**锚点漂移只告警，不阻塞**（Codex 结论：先别做成门禁，用真实项目看它是否真会漂移）：

```text
py -3 scripts/taskctl.py --root <项目根> packet TASK-001 --role worker --check-anchors
```

- 默认**不检查**；加 `--check-anchors` 时按三类分开报告，便于以后统计"锚点漂移率"不失真：
  - `ANCHOR_FILE_NOT_FOUND` —— **文件不存在**（不是锚点漂移，先确认路径）
  - `ANCHOR_NOT_FOUND` —— 锚点确实找不到（**这才是漂移信号**，让 M1 重新定位后再派工）
  - `ANCHOR_LOOSE` —— 只是格式变了，粗匹配仍命中，人工确认即可
- 粗匹配会**跳过 Markdown 前导符号**（`#`、`>`、`-`、`*`）再取第一个有意义的词，所以 `## 标题` 不会拿 `##` 去比对；
- **不进入 Full Gate、不改变任务状态**——它属于效率设计，不是监督。

**为什么值得做**：读进来的内容之后每一步都会被重发（实测 assistant 输出占重发体积 93.5%），所以"少读"是 DSH 上最直接的降本手段。

## 自测证据

每个本轮窗口必须有**可原样执行的验收命令**（检查已有文件/能否编译）。禁止只写「自测通过」。必须真跑终端。验收命令**不生成**业务文件。

| 谁 | 标准 |
|----|------|
| **工人窗** | 本回合调用终端；回复含命令 + 输出。一行 PASS 且命令本就一行 → 就是全文，可以请查收 |
| **M1 查收** | 本窗重跑该窗验收命令。用户可贴原文，但不是必须。复跑通过即过 |

## 查收（只此一份）

1. 解析点名窗号（必须是合法 M/C，见「窗口身份」）。所有窗口 = **本轮派工**名单。
2. 只核这些窗的路径、交付物、验收命令。
3. 磁盘：产出是否在规定路径。
4. 本窗重跑验收命令。通过 = **内容通过**。关联会话只当线索。
5. 启用 `.task/` 时：按策略表 `transition` 收口；`audit-round` 出现 `BASIC_GATE_PASS` + `FULL_GATE_PASS` + `ROUND_READY_TO_CLOSE` 后，M1 才能逐项 `done`。出现 `POLICY_CONFLICT`、`REQUIREMENT_COVERAGE_FAIL`、`GEAR_VIOLATION`、`HOOK_EVIDENCE_MISSING` 或 `PARALLEL_FAIL` 则停止收口，写 BLOCKERS。
6. 本轮名单都通过 → 标 done、记 RECEIPT-LOG、做合并；能开则打开网页预览。临时 C 窗 done 后视为本轮关闭。
7. 命令失败 → **fail**，打回该窗（由用户再进该窗，不会自动重开）。
8. 本轮还有窗没交 → **整轮未齐**。

禁止：仅凭口头完成、磁盘有文件、或「我看到其他对话」标 done。

**亲自微修订**：未经点名且未经同意，禁止改子模块实现。未获同意则派 M 或 C，本窗只交 Registry 与开工话术。

## 一任务两职责：worker 与 verifier（0.35）

一个任务可以同时有**一个实现负责人**和**一个独立验收人**，两者必须可区分、可审计、不可互相冒充。M1 仍是唯一收口者。

```text
TASK-001
├─ owner / worker：C1   ← 负责实现；owner 始终是 C1
└─ verifier：C2        ← 只独立验收，不接管实现
M1：唯一收口者
```

**两条路由分开存，不要混：**

| 路由 | 位置 | 含义 |
|---|---|---|
| 实现路由 | `round.json` 的 `tasks` | `C1: ["TASK-001"]`。**该窗必须等于 `manifest.owner`** |
| 验收路由 | `round.json` 的 `verifier_assignments` | `{"TASK-001": "C2"}`。仅需要独立验收的任务才有 |

**权限边界：**

| 单位 | 允许 | 禁止 |
|---|---|---|
| worker（C1） | 改 `allowed_paths` 内业务文件；写 worker 报告；推进到 `worker_done` | 写 verify 报告；自己标 done；当 verifier |
| verifier（C2） | 只读检查；写 `verify-report.json`；推进 `verifying → verified` | 改业务实现；接管 owner；验收自己的实现 |
| M1 | 派 C1/C2；本窗重跑 Gate；`integrated → done` | 替 C1 偷做实现；凭口头完成收口 |

状态路径不变：`C1: in_progress → worker_done`；`C2: verifying → verified`；`M1: verified → integrated → done`。**`verified` ≠ `done`。**

```text
py -3 scripts/taskctl.py --root <项目根> round-init ROUND-001 C1 C2 --task C1=TASK-001 --verifier TASK-001=C2
py -3 scripts/taskctl.py --root <项目根> assign-verifier TASK-001 --window C2 --actor M1
py -3 scripts/taskctl.py --root <项目根> brief TASK-001 --role verifier --window C2
py -3 scripts/taskctl.py --root <项目根> receipt C1
py -3 scripts/taskctl.py --root <项目根> receipt C2 --role verifier
```

`assign-verifier` **不是** `reassign`：它只写验收路由，**不改** `owner` / `attempt` / `block_id` / `assignment_history`。写验收路由用 `assign-verifier TASK-001 --window C2`；`--window` 在这里**专指 verifier**，worker 侧一律用 `--owner`，两个名字不混用。旧项目主路由挂错时用 `sync-worker-route TASK-001`（验收开始后不允许再改）。

**一个任务只能有一个 `owner`（主改工人）。** 需要多人协作时**拆成子任务**，或使用只读辅助 / 斥候；**禁止两个工人同时主改同一任务**。同文件并发会真的写坏产物，这是实测发生过的。

**M1 代验（0.35 明确）**：独立验收窗没开时，M1 **可以**额外复跑命令、写补充证据，但**不能冒充缺席的 verifier**，也**不能**把 `independent_verification` 悄悄改成不需要。这种情况下任务**保持未完成**。唯一的合法出口是：**在派工前**就按风险策略明确"本任务不要求独立验收"（`risk=low` 且 `attempt<2` 且未设 `verification_required`），此时走的是短路径，而不是"事后代签"。

门禁 token：`VERIFIER_ASSIGNMENT_MISSING`（没派 verifier）、`VERIFIER_ASSIGNMENT_CONFLICT`（verifier 非本轮窗 / 等于 owner 或 worker / reviewer 冒名）、`WORKER_ASSIGNMENT_CONFLICT`（`round.tasks` 的 worker ≠ `manifest.owner`）。

## 长线治理：循环渐进 + 卡点上限 3（三级阶梯）

每一环：唯一目标、成功标准、证据（diff + 本回合终端原文）、存档点。**所有任务都必须过最低门禁；规模小不等于免检。**

**同一卡点（同一根因）的三级阶梯** —— 这不是"节奏偏好"，是跨宿主稳定性规则：防止同一错误理解被无限重复。

| 第几次 fail | 动作 | 记录要求 |
|---|---|---|
| 第 1 次 | **复用原执行者**，并让它先复述打回项再动手 | 计入同一 `block_id` |
| 第 2 次 | **换真正不同的负责窗口**（或独立执行上下文），**并更新 `owner`** | 追加不可覆盖的 `assignment_history` |
| 第 3 次 | **硬停**：写 `docs/BLOCKERS/`，点名下一步 {窗号} 的【斥候\|主力\|搜剿】或 M1 / 用户 | `block_attempts` 到上限 |

**计数必须绑定 `block_id`（根因）**：
- 同一根因**不能靠改名重置**——改了 `block_id` 又不给证据，`taskctl` 直接拒绝；
- 用户新增需求，或能证明的新根因 → `--new-root --reason <证据>` 才开新卡点；
- **换人不能只在聊天里说**：必须更新 `owner`，否则等于"换了人却没换记录"；
- **宿主没有合格的新执行者时不得伪造换人**：写 `BLOCKERS` 的 `NO_ELIGIBLE_REPLACEMENT_WORKER`，交 M1 / 用户决定（DSH 子代理正处在这个情况，见 [references/dsh-evidence.md](references/dsh-evidence.md)）。

命令：

```text
py -3 scripts/taskctl.py --root <项目根> attempt TASK-001 --block-id B1 --reason "<失败证据>" --actor worker
py -3 scripts/taskctl.py --root <项目根> attempt TASK-001 --block-id B1 --reason "<打回项>" --actor worker --owner C1
```

## 性能纪律（0.33 新增，跨宿主）

Token 放大的大头**不是门禁**（实测账本/hook 事件 < 0.1%），而是**步数 × 模型自己写的文字量**（实测 assistant 输出占重发体积 93.5%）。以下四条只压效率，不动验收：

1. **回执预算**：成功回执 10～20 行，格式固定为「状态 + 路径 + 关键行号/片段 + 验收命令 + **命令输出原文** + 未决项」。**上限只管叙述，命令输出原文必须完整保留**（它是证据）。失败/阻塞可超限，但须说明原因。
2. **有界读取**：任何 > 100 行或 > 4KB 的文件，只允许"行号区间 / 关键词片段"读取；整篇读取要写明理由。
3. **M1 状态驱动输出**：M1 只在**新决策或状态变化**时输出控制信息，不重讲已确认背景；但 **Gate fail、越界、需求冲突等异常必须立即处理，不能等到轮次结束**。M1 的需求对齐、派工、验收、收口**不压缩**。
4. **斥候按需触发**：判据是"**我知不知道文件在哪**"，不是"任务大不大"。
   - 已知路径 + 已知影响范围 + 信息未过期 → **不开斥候，直接主力**（但仍受第 2 条有界读取约束）；
   - 已知路径 + 影响范围不确定 → **微型斥候**：只查关键依赖，短回报；
   - 路径未知 / 注册表过期 / 文档与现状冲突 / 陌生项目 → **受限斥候**：只读、限长、给路径与风险。

**注意（防矫枉过正）**：信息未知时跳过斥候会更贵——主力的探索是"读+写"混合的，过程留在主线被反复重发，还可能已经改错文件。斥候的成本是**一次**，主力乱翻的成本是**每次重发都在付**。

## 工具纪律（0.35 新增，实测踩坑；**进文档不进状态机**）

以下四条来自三轮真实交付的复盘，都属于"证据会被工具链骗"的类型，因此**写成纪律，不写成门禁**：

1. **不用 shell 整文件改写源码。** `-replace | Set-Content` 这类整文件替换会吃掉注释、把代码写进注释里，而 `node --check` **仍然通过**——工具会告诉你没事。改代码用精确编辑（带上下文的替换），改完再单独跑语法检查。
2. **生成器也是交付物。** `patch-*.mjs` 之类的脚本会把**已经修好的产物打回旧版本**。收口前必须检查：仓库里还有没有会把修复覆盖掉的脚本？有就一起修或一起删，别只修产物。
3. **改契约时，旧断言失败 ≠ 业务回退。** 若任务本身改的就是契约，派工时必须**点名哪些旧断言要一起更新**。旧断言红了先看它是不是被契约变更淘汰，不要直接当成实现错误回滚。
4. **探针会骗人：读数异常先怀疑测量，再怀疑实现。** 具体表现（都实测过）：采样拍频造假象、`.click()` 不派发 `pointerdown`、偶发抖动。纪律：真实输入用宿主真实输入 API（不用 `.click()` 代替），等动画结束，隔离干扰变量，**对偶发红项先复跑确认**再判定。

## 项目文档

M1 创建：`docs/MODULE-REGISTRY.md`（路径、交付物、验收命令）、`docs/RECEIPT-LOG.md`（查收叙事）、`docs/FIX-PLAN.md`；可选 `docs/BLOCKERS/`、`docs/TOOL-PATHS.md`。启用 `.task/` 后还必须生成 `docs/TASK-STATUS.md`：

```text
py -3 scripts/taskctl.py --root <项目根> status --markdown --write
```

**状态单一来源是 `.task/`。** `TASK-STATUS.md` 只是视图，禁止手改，只由上面的命令再生。Registry 总览表不要再手写状态列；RECEIPT-LOG 可以记本窗重跑原文，但 **结论 done / pending 以生成表和 `transition` 为准**。手写 Registry `done` 不能代替 `transition ... done`。

任务是否完成以 manifest 的 `transition` 为准。窗口状态仍在 `round.json` 的 `window_status`，不要把 `C1 done` 写进 round 冒充任务完成。

## 依赖、并行、集成

```
M1 定标准 → 常驻骨架模块先过查收 → 其余常驻 M 可并行；临时 C 可与常驻并行（各守文件） → M1 集成
```

并行只提高效率，不是新工作流：

- 只用**当前宿主已经提供**的多代理 / 子代理。宿主没有就不要强用，不要发明新窗口角色或新编号（见「窗口身份」）。
- 默认流程仍是用户开 M/C 窗并传查收。子代理不是窗号；活必须落在 Registry 已有 M/C 任务和 `allowed_paths` 上，并写入 `subagents`（`task_id` / `window` / `allowed_paths` / `run_id`）。
- 同一文件仍只让一个任务主改。不能用同一个子代理既当工人又当独立验收。
- 子代理回传说完成，只当查收**信号/线索**。M1 仍须本窗重跑，仍须 `transition` 收口。
- 未确认本窗有额外能力时，不要少开窗、少等用户传话，也不要说加分，协作挡保持 P。

| 原则 | 说明 |
|------|------|
| 同一文件只让一个窗主改 | 边界写死路径 |
| Phase 2 修复 | 复用已有 M2～M10 或开临时 C；窗号见「窗口身份」 |
| 修 Bug | 可叠加 `/bugfix`；仍受同一卡点 3 次阶梯约束（见「长线治理」） |
| 配置冲突 | M1 合并 |

## 反模式

- 发明第三套窗号，或用集合外标识当窗号（见「窗口身份」）
- 一轮临时 C 超过 4 个，或临时窗当长期部门
- 以为工人跑完验收会自动交给 M1，或 fail 会自动打回重做
- 把验收命令当成「生成文件的脚本」
- 未确认加分却说「当前是加分状态」，或凭「产品有子代理」自行升为加分
- 未点名加分却用「已看到其他对话」标 done
- 把加分问句写进 Mn/C 开工话术
- 引用 CHANGELOG 里的旧版块当当前规则
- 整份覆盖 `manifest.json`，或把 `transition` 改过的 manifest 漏报当成必须打回的业务文件
- 让工人申报 `verify-report.json`，或让 C1 改 `worker-report.json` 来过 G2
- 子窗口自行标 done；没跑终端只编 PASS；子窗口打开网页预览
- M1 未经点名、未经同意就亲自改子模块
- 低风险短路径与独立验收两条路同时走，或忽略 `POLICY_CONFLICT` 继续收口
- 派工时手写转述代替 `brief` 生成物，或新 M1 不跑 `handoff` 凭记忆接班
- 手改 `docs/TASK-STATUS.md`，或用 Registry / RECEIPT-LOG 的 pending/done 覆盖 `.task/`
- 用户口头加需求却不写 `source_refs` / `source_requirements`，或映射空的 `maps_to` 仍标完成
- 未确认却自报加分 / 协作挡 A，或 `gears.capability=bonus` 但 `capability_confirmed` 为假
- `hook_supervision=true` 却用手动 `audit-round` / `--source manual` 冒充 hook 证据
- 派出子代理却不写 `subagents`，或同文件并发 / 工人兼 verifier / 越权路径仍收口
- 加分或 A 挡漏跑独立验收，仍把任务标完成
- 旧项目直接 `--force` 覆盖 `taskctl.py` 却不备份、不写报告 / `skill-lock.json`
- 迁移后未 `MIGRATE_READY` 就开始开发新功能；无 `--force` 时覆盖已有脚本；把正在运行的脚本迁到自己身上
- 手工删除 `round.json` 开新轮，或用 `round-close` 归档还有未完成任务的轮
- 把只读 `status` 当收口依据，或把 `STALE_REPORT`（软提示）当成任务失败去打回
- `tests[].command` 里写临时探针、占位符或带说明文字的命令；把探针结论从 `note` 挪进 `tests[]`
- 读到 `UNSUPPORTED_SCHEMA` 仍继续改状态 / 收口，或为了让旧脚本跑通而手工下调 `schema_version`
- 把未完成交付从 `known_gaps` 挪到 `notes` 后继续收口；用 M1 代签掩盖 verifier 窗口缺席
- 让两个窗口共同主改一个任务（一个任务只能有一个 `owner`）
