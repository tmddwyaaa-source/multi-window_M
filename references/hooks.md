...
Codex 使用项目级 `.codex/hooks.json` 或用户级 `~/.codex/hooks.json`。建议先使用项目级配置。把 `{SKILL_ROOT}` 换成本机 Codex 技能目录（升级系列隔离副本为 `...\skills\multi-window-m-036`）：

```json
{
  "description": "multi-window-m-036 task audit",
  "hooks": {
    "Stop": [
      {
        "hooks": [
...
隔离副本的 `SKILL_ROOT` 例：`C:\Users\user\.cursor\skills\multi-window-m-036`。
