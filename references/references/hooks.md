...
Codex 使用项目级 `.codex/hooks.json` 或用户级 `~/.codex/hooks.json`。建议先使用项目级配置。把 `{SKILL_ROOT}` 换成 `C:\Users\user\.codex\skills\multi-window_M`：
...
```text
py -3 C:\Users\user\.cursor\skills\multi-window_M\scripts\taskctl.py --root <工作区> hook-audit --source cursor-stop --host cursor
```
...
```text
py -3 C:\Users\user\.cursor\skills\multi-window_M\scripts\taskctl.py --root <项目根> hook-audit --source zcode-stop --host zcode
```
